...
# Base landslide sensors (Coordinates + Weight Intensity factor for the Heatmap)
landslide_sensors = [
...
# Infrastructure Assets needing algorithmic prioritization
...
# Multi-select options for risk visualization layers
...
col1, col2 = st.columns([2, 1]) # 2:1 Split-screen for optimized layout mapping

with col1:
    st.subheader("🌐 Dynamic Spatial Risk Map")
    
    # Initialize Map Canvas
    map_center = [30.3500, 79.2000] # Broad regional center for Uttarakhand clusters
    sih_map = folium.Map(location=map_center, zoom_start=8, tiles=None)

    # Core Connection to Google Maps Tiles APIs
    google_tiles = {
    ...
    if show_heatmap:
        # Extract coordinates and intensity weights [lat, lon, weight]
    ...
    for asset in infrastructure_assets:
        if asset["type"] in selected_asset_types:
            ...
            """
            # Add pinpoint markers matching traditional GIS systems
            folium.Marker(
    ...
    # Display Map
    st_folium(sih_map, width="100%", height=600)

# --- 5. Downstream Infrastructure Analytics & Prioritization Matrix ---
with col2:
    ...
    # Convert asset dictionaries to Pandas DataFrame for high-performance sorting
    df_assets = pd.DataFrame(infrastructure_assets)
    
    # Filter DataFrame based on sidebar selections
    df_filtered = df_assets[df_assets["type"].isin(selected_asset_types)]
    
    # Custom categorical sorting: Critical Priority -> Watch List -> Safe / Stable
    ...
    # Render Prioritization list inside UI card widgets
    for _, row in df_filtered.iterrows():
        # Match alerts style to hazard colors
